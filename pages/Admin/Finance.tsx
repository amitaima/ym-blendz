...
const AdminFinance: React.FC = () => {
  ...
  return (
    <div className="p-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 text-right">
      ...
      {showAdd && (
        <div className="glass-card p-6 rounded-2xl border-gold/20 border space-y-4">
          <h3 className="text-sm font-bold uppercase tracking-widest text-gold-light">הוסף הוצאה</h3>
          <div className="space-y-3">
            <input 
              type="text" 
              placeholder="כותרת (למשל, מכונת תספורת חדשה)"
              className="w-full bg-white/5 border border-white/10 rounded-xl p-3 focus:border-gold outline-none text-sm text-right"
              value={newExp.title}
